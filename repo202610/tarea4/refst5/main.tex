...
Dado un grafo $G$ y dos nodos $u,v$ en él, el problema de optimización conocido
como \texttt{Longest Simple Path} consiste en encontrar un camino simple (que
no repite nodos) y de longitud máxima que los conecta. Denotamos este problema
con 
\(
	\llbracket
	\mathtt{LSP}
	\rrbracket.
\)
El problema de decisión asociado, que denotamos con \texttt{LSPD}, está dado por
$X \subset \hat{X}$ donde
\(
	\hat{X}= \mathtt{Graphs} \times \ZZ \times \ZZ \times \ZZ,
\)
y
\[
	X=
	\begin{array}{l}
	\{
		(G, u, v, k) \vert \ \mbox{existe un camino simple en $G$}\\
		\mbox{\ \ desde $u$ hasta $v$ con al menos $k$ nodos intermedios}
	\}
	\end{array}
\]
\end{document}
