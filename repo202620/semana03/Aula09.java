public class Aula09 {
	 ...
	 */
	int buKnapsack(int[] ws, int[] vs, int maxW) {
		int n= ws.length;
		int[][] subs= new int[n+1][maxW+1];
		for (int i= 1; i <= n; i++) {
			int wi= ws[i-1];
			int vi= vs[i-1];
			for (int j= 1; j <= maxW; j++) {
				...
				subs[i][j]= Math.max(subs[i-1][j], vi +
						subs[i-1][j-wi]);
			}
		}
		return subs[n][maxW];
	...
	/**
	 * @param ws Weights. Positive
	 * @param vs Values. Positive.
	 * @param subs Matrix of subproblems' solutions
	 * @param j Item index, in 0..n-1
	 * @param c The capacity of the Knapsack
	 */
	int tdKnapsack(int[] ws, int[] vs, int[][] subs, int j, int c) {
		if (subs[j][c] > 0) {
			return subs[j][c]; // Return if solution already computed
		}
		int cuv= vs[j]; // Value of current Item
		int cuw= ws[j]; // Weight of current Item
		if (cuw > c) {
			return tdKnapsack(ws, vs, j-1, c); // Current item is too heavy
		}
		int prev1= 0;
		if (subs[j-1][c-cuw] > 0) {
			prev1= subs[j-1][c-cuw];
		}
		else {
			prev1= tdKnapsack(ws, vs, j-1, c-cuw);
		}
		int sol1= prev1 + cuv; // Best solution INCLUDING current item
		int prev2= 0;
		if (subs[j-1][c] > 0) {
			prev2= subs[j-1][c];
		}
		else {
			prev2= tdKnapsack(ws, vs, j-1, c);
		}
		int sol2= prev2; // Best solution WITHOUT current item
		int sol= Math.max(sol1, sol2);
		subs[j][c]= sol; // Update solutions table
		return sol;
	}
